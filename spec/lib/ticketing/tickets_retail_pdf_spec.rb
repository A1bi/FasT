...
RSpec.describe Ticketing::TicketsRetailPdf do
  let(:ticket_medium) { 'retail' }
  let(:page_layout) { [595, 280] }
  ...
  shared_examples 'renders the correct ticket information' do
    ...
    it 'renders the correct ticket barcodes' do
      order.tickets.size.times do |i|
        ...
        expect(tickets_pdf).not_to receive(:link_annotate).with(authenticated_content(i), any_args)
      end
      pdf
    end
  end
