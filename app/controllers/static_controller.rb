...
class StaticController < ApplicationController
  ...
  def index
    @featured_events = Ticketing::Event.where(identifier: %i[canterville])
    @featured_galleries = Rails.env.development? ? [Gallery.last] * @featured_events.count : Gallery.where(id: [77])
